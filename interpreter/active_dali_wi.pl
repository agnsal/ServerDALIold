...
estrapola_param1(Me):-open('../interpreter/server.txt',read,Stream,[]),
	  read(Stream,T),
	  close(Stream),
...
%% EDITONTO Aggiunto nuovo sistema per le ontologie.
...
%load_ontology_file(Ontolog):-open(Ontolog,read,Stream,[]),
%                                   repeat,
%                                         read(Stream,T),
%														
%                                         assert((T)),
...
start1(F,Fe,N,Libr,Fil):-
  ...
  append(L,[101],T),
  name(S,T),
  %%EDITED Eliminato "delete_file/2" non è nel sicstus4
...
%%EDITED aggiunto trace
%L0 dovrebbe esser communicationf/fipa			
libreria(F,L0,Fil):-name(F,Lf),append(Lf,[46,112,108],Ltf), %../program/cane = F1
	   append(L0,Fil,L),
	   name(F1,Ltf),if(L=[],true,libreria1(F1,L)). %%cane e communication, planasp etc
libreria1(F,L):-last(L,U),
		   repeat,
			  member(Me,L),
				  appendi_regole0(Me),
		   Me==U,!,versa(F). %%versa scrive nel pl tutte le dir. di communication, planasp etc.
...
appendi_regole(Mef):-open(Mef,read,Stream,[]),
			   repeat,
				...
				T==end_of_file,!,
		close(Stream).
%%Scrivo il communication. il planasp eccetera tutti dentro al file pl
...
examine1_mul:-findall(L,clause(mul(L),_),S),%%L in mul(L) è un PREDICATO EVE LISTIFICATO!!
              last(S,E),
...
%%QUi una volta ebbi messo un trace
cont_tep1(L,T):-findall(X,clause(tep(_,X),_),Le),retractall(tep(_,_)),
		if(clause(fatto_mul(L,Le),_),true,call(T) ). %%chiama "eve di qualcosa"
...
%'EdITED -- L'end_of_file è trattato diverso nel 4. aggiunt (if(L\=end_of_file blabla )
...
% ASSORBIMENTO DELLA ONTOLOGIA DELL'AGENTE COMUNICANTE
%asserisci_ontologia(AgM,Ontology,E):-if(Ontology=[],true,asserisci_ontologia1(AgM,Ontology,E)).
%asserisci_ontologia1(AgM,Ontology,E):-last(Ontology,U),
%                                       repeat,
%                                         member(Me,Ontology),functor(E,F,_),
%                                         asse_cosa(ontology(AgM,F,Me)),
%                                       Me==U,!.
...
%%PROBLEMA! Qui a un certo punto arriva [] EDIT
...
%%EDITONTO eliminato invia_terms_ontology(Fun,L)
...
%% Azione di invio del messaggio
%emetti_ms(To,M,S,IndS,IndTo):-clause(own_language(Lang),_),arg(1,M,A1),
%                              functor(A1,Fun,_),invia_terms_ontology(Fun,L),
%                              out(message(IndTo,To,IndS,S,Lang,L,M)),
%                              print(send_message_to(To,M,Lang,L)),nl,
%							  save_on_log_file(send_message(M)).



%%EDITONTO Nuovo sistema di gestione delle ontologie
%% MANDO SOLO LA MIA ONTOLOGIA.
invia_terms_ontology(O):-
	clause(agent(Agent),_),
	if(clause(ontology(Prefixes,[Rep,Host],Agent),_), O=ontology(Prefixes, [Rep,Host], Agent), O=[]).

%invia_terms_ontology(S,L):-clause(agent(Ag),_),
%                           findall(X,(clause(ontology(Ag,S,X),_);clause(ontology(Ag,X,S),_)),
%                           L).
...
%manda_a_room(M,S,IndS):-arg(1,M,A1),arg(2,M,Cat),functor(M,F,_),
%                       datime(C),
%                       functor(A1,Fun,_),
%                       invia_terms_ontology(Fun,L),
%                       clause(own_language(Lang),_),
%                              out(room(F,Cat,M,IndS,S,Lang,L,C)),
%                              print(send_desire(F,Cat,M,IndS,S,Lang,L,C)),nl. 
...
%% è chiamato dalla "take_meta_var" contenuta in "togli_var.pl"
...
%%EDITED!! Ma può mai sopravvivere un evento multiplo fino alla stesura dello plv?
%%Si' SOPRAVVIVE!! ' Perchè nel plv gli eventi multipli vanno riscritti come clausole separate

%%EDITATO , ho cambiato messo "L_eve\[]".
...
duplicate_file_profile(U,P,S):-
 ...
 generate_agent_assistant(P,U):-
	   atom_concat('C:\\Programmi\\SICStus Prolog 3.12.7\\bin\\spwin.exe -l ',P,K),atom_concat(K,'active_dali_wi.pl --goal start0(\'',K1),
	   atom_concat(K1,P,K11),atom_concat(K11,'initialization_dali_files/',K2),
	   number_in_term(K2,U,K3),atom_concat(K3,'.txt\').',K4),exec(K4,[std,std,std],_).
