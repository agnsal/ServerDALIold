...
leggiFile(Infile,Txt) :-			%apertura file,lettura righe
					%e chiusura file
	atom_concat(Infile,'.txt',File),	%...sostituito col seg. piu` casereccio
	see(File),   
	leggiChars(Txt), !,
	seen.
...
leggiFile_fil(Infile,Txt) :-			%apertura file,lettura righe
					%e chiusura file
	atom_concat(Infile,'.con',File),	%...sostituito col seg. piu` casereccio
	see(File), 
	leggiChars(Txt), !,
	seen.

%leggiChars(R,T) :- %lettura di un pezzo, carattere per carattere e...
%	trace, skipBlanks(Bl,NoBl), 		 %2 codici ascii
%	( charEof(NoBl) -> R=[], T=[]; %chiusura della lista se trovato l'eof
%	                %..inserimento nella lista dei char
%	  ( Bl==NoBl -> R=[Bl|SS]; %%'%Bl == NoBl vuol dire che non era ne'' un blank ne'' un eol nella skipblanks. 
%			    R=[Bl|S], S=[NoBl|SS] ) %% tira fuori tutti i nl e i blank consecutivi e pure il primo NonBlank.
%	  , leggiChars(SS,T)
%	).
